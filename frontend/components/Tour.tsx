...
const Tour = () => {
  ...
  // const
  const startTourTimeout = useRef<NodeJS.Timeout>();
  const handleJoyrideCallback = useCallback((data: CallBackProps) => {
    // const { tour, startTour } = useContext(TourContext);
    const { status, type, action, index } = data;

    const nextStepIndex = index + (action === ACTIONS.PREV ? -1 : 1);
    console.warn("nextStep",nextStepIndex, "Action: " + action, "type: " + type);
    console.warn(tour)
    if(tour.steps.isRoute  ){
        console.log(tour.steps)
    }
    if (type == "step:after") {
    //   clearTimeout(startTourTimeout.current);
    //   startTourTimeout.current = setTimeout(() => {
        startTour({
          run: true,
          tourActive: true,
